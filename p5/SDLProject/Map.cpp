...
void Map::Build() {
    for(int y = 0; y < this->height; y++) {
        for(int x = 0; x < this->width; x++) {
            ...
            if (tile == 0) continue;
            float u = (float)(tile % tile_count_x) / (float)tile_count_x; float v = (float)(tile / tile_count_x) / (float)tile_count_y;
            float tileWidth = 1.0f/(float)tile_count_x; float tileHeight = 1.0f/(float)tile_count_y;
